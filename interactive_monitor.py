...
class InteractiveMonitor:
    ...
    def get_sort_display(self):
        """取得排序方式顯示文字"""
        sort_map = {"ward": "地區/房號", "price": "價格", "entries": "抽籤人數"}
        return sort_map.get(self.sort_by, "地區/房號")

    def format_plot(self, plot: PlotState) -> str:
        """格式化房屋資訊"""
        # 房屋大小名稱
        size_names = {HouseSize.SMALL: "小", HouseSize.MEDIUM: "中", HouseSize.LARGE: "大"}
        ...
        col3 = f"{size_names.get(HouseSize(plot.size), '?')}"
        ...
        if plot.lotto_entries is not None:
            ...
            if plot.lotto_phase is not None:
                phase = phase_display.get(LottoPhase(plot.lotto_phase), "❓ 未知")
            else:
                phase = "❓ 未知"
