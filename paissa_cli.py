...
def filter_plots(plots: List[PlotState], args) -> List[PlotState]:
    ...
    # Filter by district
    if args.district:
        filtered = [p for p in filtered if args.district.lower() in p.district_name.lower()]
    ...
    if args.max_entries is not None:
        filtered = [p for p in filtered 
                   if p.lotto_entries is not None and p.lotto_entries <= args.max_entries]
...
async def main():
    ...
    # List worlds command
    parser_worlds = subparsers.add_parser('worlds', help='List all available worlds')
