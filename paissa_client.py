...
class PaissaClient:
    ...
    async def load_world(self, world_id: int, force_reload: bool = False) -> Optional[WorldDetail]:
    ...
    def get_district_plots(self, world_id: int, district_id: int) -> List[PlotState]:
        ...
        return [
            plot for plot in self.plot_states.values() if plot.world_id == world_id and plot.district_id == district_id
        ]
            ...
            for district in world_detail.districts:
                if district.num_open_plots > 0:
                    ...
                    for plot in district.open_plots[:5]:  # Show first 5 plots
                        ...
                        print(
                            f"  - Ward {plot.ward_number} Plot {plot.plot_number} " f"({size_name}, {plot.price:,} gil)"
                        )
