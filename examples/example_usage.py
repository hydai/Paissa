...
from paissa_client import HouseSize, LottoPhase, PaissaClient  # noqa: E402
...
async def example_basic_usage():
    ...
    async with PaissaClient() as client:
        ...
        for plot in plots[:5]:
            ...
            print(
                f"Ward {plot.ward_number} Plot {plot.plot_number} - "
...
async def example_lottery_monitoring():
    ...
    async with PaissaClient() as client:
        ...
        lottery_plots = [p for p in plots if p.lotto_phase == LottoPhase.ENTRY]
...
async def example_find_cheap_plots():
    ...
    async with PaissaClient() as client:
        ...
        for world, plot in cheap_plots[:10]:  # Show cheapest 10
            print(f"{world.name} - Ward {plot.ward_number} Plot {plot.plot_number}: " f"{plot.price:,} gil")
