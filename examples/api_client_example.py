...
class PaissaAPIClient:
    ...
    def get_world_plots(
        ...
        response = self.session.get(f"{self.base_url}/api/worlds/{world_id}/plots", params=params)
        response.raise_for_status()
        result: List[Dict[str, Any]] = response.json()
        return result

    def get_world_stats(self, world_id: int, force_refresh: bool = False) -> Dict[str, Any]:
        """Get statistics for a world"""
        params = {"force_refresh": force_refresh}
        response = self.session.get(f"{self.base_url}/api/worlds/{world_id}/stats", params=params)
        response.raise_for_status()
    ...
    def search_plots(
        ...
        params: Dict[str, Any] = {"max_price": max_price, "lottery_only": lottery_only, "limit": limit}
...
def main():
    ...
    try:
        ...
        plots = client.get_world_plots(world_id, size="small", max_price=3500000, available_only=True, limit=10)

        print(f"Found {len(plots)} small plots under 3.5M gil:")
        for plot in plots:
            print(
                f"  - {plot['district_name']} W{plot['ward_number']:02d} "
        ...
        cheap_plots = client.search_plots(
            max_price=3000000, size="small", worlds=[34, 35, 36], limit=10  # Multiple worlds
        )
        ...
        for plot in cheap_plots:
            ...
            print(
                f"  - {world_name}: {plot['district_name']} "
        ...
        for info in cache_info:
            print(f"  World {info['world_id']}: " f"{info['plot_count']} plots cached at {info['cached_at']}")
